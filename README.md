...
Markdown explains the skill. **YON makes it inspectable and enforceable** — the control flow, the rules (`MUST` / `MUST_NOT`), and the gates (`ABORT` / `WARN`) are named, typed objects, not prose you hope the model follows. Of the 49 skills here, **35 ship a `protocol.yon`**; 14 are Markdown-only.
...
Browse [`skills/`](skills/) for the full set of 49 — planning, insight & decision, [orientation](#the-orient--family), priming, orchestration, code & architecture, Obsidian/vault, web extraction, git, and YON authoring.
