...
The mandatory CORE/FULL protocol remains complete on its own. Optional operating packages load lazily after the handshake: a bounded Collab Window for Work-or-Listen, native Scheduled Collab when the runtime can prove a bounded scheduler, and mandatory missed-message recovery when detection or cursor evidence contradicts readiness. Collab Window and Scheduled Collab are participant-local stances: each agent chooses its own cadence and horizon without proposing, countering, or waiting for peer acceptance. Concrete peer duties remain ordinary scoped mailbox requests. A package failure blocks only that local capability; the base collaboration stays usable.

Every valid addressed message is a call to action with a durable disposition, including an explicit `no-reply-required` outcome when wire silence is correct. The consumed-UUID cursor is a compact checkpoint, not sole proof of handling. Reported misses revoke `LISTENING`, quarantine ambiguous history, and require evidence-backed recovery before readiness returns. Returning agents can reuse an established room without replaying its old conversation: `resume → state` rehydrates the seat, then new work starts from a fresh CTA root while legacy mode proposals are disposed without restarting negotiation.
