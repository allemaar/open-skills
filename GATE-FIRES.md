# Gate-fires maintainer diagnostic snapshot
...
The table below records one historical diagnostic run against deliberately broken fixtures. It
shows only what those commands reported in that run. It does not prove runtime obedience,
current product correctness, or that every guard still behaves identically.

**Snapshot result:** the listed deliberately broken fixtures were rejected in that run.
