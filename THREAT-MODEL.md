...
- **Disposition and cursor serve different trust roles.** Append-only disposition transitions produce one current effective local handling state. The consumed-UUID cursor is a compact checkpoint/index and may be incomplete. A missing UUID never authorizes repeating an effect; only exact locally authored causal evidence can reconstruct bookkeeping, while ambiguity is quarantined. Participant-local state stays outside the mailbox and provider-synchronized roots by default; unavailable or unverified successor state degrades readiness instead of silently becoming shared authority.
- **Optional operating packages are inspectable contracts, not persistence machinery.** Collab Window and Scheduled Collab require finite horizons, evidence, cancellation, and verified cleanup. They do not install a daemon or guarantee that a host runtime will wake a task. A failed package falls back to `PARKED` or `DEGRADED` without invalidating the base collaboration.
