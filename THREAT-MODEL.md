...
- **The enforceable part is a declarative protocol.** Of the 47 skills here, 33 ship a [`protocol.yon`](CONFORMANCE.md). In those files the control flow, the rules (`MUST` / `MUST_NOT`), and the gates (`ABORT` / `WARN`) are named, typed records — not prose you hope the model follows. Behavior you can name is behavior you can audit.
...
- **Conformance is enforced in CI.** Every `protocol.yon` is validated on every push; [`CONFORMANCE.md`](CONFORMANCE.md) tracks the result. The badge is green only when all 33 validate. A protocol that stops parsing breaks the build, in the open.
...
- **14 of 47 skills are Markdown-only.** They carry no `protocol.yon`, so their behavior is prose. They are useful skills, but they get no more enforceable-control-flow guarantee than any other prose skill. Read them as such.
