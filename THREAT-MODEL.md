...
Most skills travel as opaque bundles or as prose you skim once. open-skills takes the opposite stance: every skill is plain text you can read before you trust it, and 40 of 68 add a declarative protocol you can inspect and validate with a public tool. Inspection is the defense. This document explains what that defends against, and — just as important — what it does not.
...
- **28 of 68 skills are Markdown-only.** They carry no `protocol.yon`, so their behavior is described only in prose. The 40 dual-document skills add a checkable declaration, not a runtime-obedience guarantee. Read both forms as instructions granted your agent's permissions.
...
Agent Mailbox piggybacks on a Handler-controlled folder transport. It does not authenticate participants, sandbox peer content, or create a trusted execution channel.

- **A message is a call to action, not authority.** Every valid addressed message requires a durable disposition, but peer content cannot expand publication, destructive, financial, credential, external-write, or project scope. An out-of-scope request produces one informed Handler gate; a specific approval authorizes only the recorded expansion and leaves higher runtime/repository gates intact.
- **Disposition and cursor serve different trust roles.** Append-only disposition transitions produce one current effective local handling state. The consumed-UUID cursor is a compact checkpoint/index and may be incomplete. A missing UUID never authorizes repeating an effect; only exact locally authored causal evidence or independently verified effect/idempotency evidence tied to the request can reconstruct bookkeeping, while ambiguity is quarantined. Participant-local state stays outside the mailbox and provider-synchronized roots by default; unavailable or unverified successor state degrades readiness instead of silently becoming shared authority.
- **Optional operating packages are inspectable local contracts, not peer commitments or persistence machinery.** Collab Window and Scheduled Collab require finite horizons, evidence, cancellation, and verified cleanup, but each participant configures its own cadence and horizon without peer negotiation. Concrete peer duties use ordinary scoped CTAs. The packages do not install a daemon or guarantee that a host runtime will wake a task. A failed package falls back locally to `PARKED` or `DEGRADED` without invalidating the base collaboration, participant lifecycle, or conversation.
- **Room history is not current authority.** A returning holder rehydrates an established room with `resume → state`, then starts new work from a fresh CTA root. Legacy mode proposals are sender-local advisory metadata, not an invitation to recreate their acceptance or renewal loop. Ambiguous non-mode history remains quarantined debt.
- **Transport, detection, and wake are separate.** Local folders, Git, free Lyt (Link Your Think™) vaults, and sync-share folders such as OneDrive can carry mailbox files. Successful publication or local materialization does not prove detector correctness, message consumption, task wake, or re-arm.
- **Public and shared evidence is minimized.** Validation, examples, release material, screenshots, and videos use sanitized data. Private paths, participant/project identifiers, operational UUIDs, exact private timestamps or hashes, raw logs, and private task text do not belong in the public repository. A shared primer uses opaque aliases and root-relative references; resolved absolute host paths remain participant-local.

YON keeps these rules inspectable. It still does not force an agent runtime to obey them.
