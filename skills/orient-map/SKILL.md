...
name: orient-map
description: >
  Delta-first "show me the shape" read on any subject — what changed since your last look, plus done → here → next as a path on trunk or a TREE on a branch, computed fresh via a bounded read-only subagent. Not /orient-status (position + remaining + ETA), an IDE file-tree, or /plan-deep-dive — orient-map shows the SHAPE and the DELTA.
visibility: public
...
6. **Emit the triple bundle + the family footer.** See *Record emission* below for the YON face; the markdown read follows the worked examples; the **visual face is optional (a node/edge graph) but its ASCII twin is mandatory and always emitted** — it is the only render in a no-SVG channel and must carry every node, edge, and change the visual would. On a branch, **ETA refuses to forecast the trunk** (`trunk eta: n/a — not on this path`). The footer is one evidence-derived line — and **only ever suggests a skill that is actually installed** (fall back to a plain prose hint or `/orient-status`/`/plan-create` otherwise). The footer's four faces, the **staleness short-circuit** (anchor unchanged → "no change" without a full sweep), the neutral re-look signal, and handoff-feeder routing follow the shared [`orient-spec/family-behaviors.md`](../../orient-spec/family-behaviors.md).
...
- MUST emit a record conformant to `orient-spec/orient-record.yon` (reserved tags; tree nodes as `@CFG id=node.<x>`; list fields as sidecar `@MAP`; never `yon format` an instance).
