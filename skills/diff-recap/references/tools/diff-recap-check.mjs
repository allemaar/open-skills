...
//
// Usage:
//   node <skill-dir>/references/tools/diff-recap-check.mjs                          # self-test: example PASS, bad fixtures REJECTED
//   node <skill-dir>/references/tools/diff-recap-check.mjs <instance.yon>           # validate one record; exit 0 = ok, 1 = not
//   node <skill-dir>/references/tools/diff-recap-check.mjs <instance.yon> --numstat <numstat.txt>   # also verify rows vs git numstat
//   node <skill-dir>/references/tools/diff-recap-check.mjs -- <path-starting-with-dashes.yon>       # -- ends flag parsing
//
...
const SCHEMA = path.join(ROOT, 'recap-schema.yon');
