...
Each participant writes only under its own portable, validated, settled `workspace/<CALLSIGN>/{artifacts,scratch}` tree; provisional claimants create no workspace. Peers may read but never write or repair there. A displaced claimant's existing tree remains attributed to its creator and becomes `needs-audit: callsign-collision`; no participant, including the winner, may inherit or ordinarily write it until exact-path remediation closes the debt. `artifacts/` targets are create-once; `scratch/` is owner-mutable without Handler remediation, disposable, never delivered or referenced, still subject to path safety and the room's sensitivity/no-secrets rules, and returns to an empty baseline before `goodbye`. Workspace presence is not a CTA: delivery requires a causal inbox message with the workspace-relative path and full hash. Pen transfer maps an existing predecessor artifact to an immutable successor continuation through causal `deliver`; with no predecessor artifact the successor sends a causal `ack`, while missing, ambiguous, or scratch-only evidence becomes `needs-audit`. A current Handler may authorize a finite named custodian to remediate exact collision, orphan/retirement, blocked-departure scratch, or privacy-incident paths—including exact existing targets—without granting general ownership. The authorization and pre-action check bind source type plus file or canonical tree-manifest fingerprint; recursive actions inventory all descendant reparse points and never follow redirecting or unknown links; move-quarantine names one exact contained absent destination. A changed or removed delivered artifact gets a causal remediation notice and primer/review invalidation. Unsafe workspace state blocks only the affected workspace write; a separately safe inbox remains usable. Any legacy flat `artifacts/` directory remains read-only unless ownership is proved by causal messages, primer provenance, or current Handler direction, and must not remain a second active artifact home.
