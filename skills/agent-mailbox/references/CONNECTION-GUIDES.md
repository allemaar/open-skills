...
Monitor health cannot depend on the Monitor reporting its own death. Persist the expected next heartbeat and bounded grace outside the shared room. On every notification, Handler return, work-boundary check, rearm, and readiness claim, require a newer heartbeat by that bound; otherwise mark `DEGRADED` and perform the normal gap reconciliation before any separately authorized rearm.

### Codex adapter

The ordinary Codex default is **Post-turn Scheduled Collab** through a host-native bounded heartbeat or equivalent bound to the exact Handler-visible task in which collaboration was started. Before registration, surface that task to the Handler. Do not create, fork, or select a dedicated collaboration task unless the Handler explicitly requests one, and do not present a listener menu. If exact same-task binding and re-entry cannot be proved, report `PARKED` and use Standard Exchange.

Each firing re-enters that surfaced Handler-visible task with one compact instruction, reloads durable mailbox state, performs the transport-required materialization or sync, reconciles the complete addressed inbox, handles each valid CTA once, persists dispositions before advancing the cursor, records the result, and exits. When a request is found, keep its project work, progress commentary, gates, and result in the same task; do not hand it to a child or standalone automation task unless the Handler explicitly directs delegation. Use an absolute deadline, maximum firing count, no-overlap identity, failure budget, observable result, and verified deletion or disablement at expiry. A firing proves task re-entry only; it does not prove provider materialization or that a message exists.
...
Active-turn polling may reduce latency while the turn remains open but cannot justify post-turn `LISTENING`. `codex exec resume` is CLI/headless continuation, not Desktop visible-task wake proof. The official [`codex app-server`](https://github.com/openai/codex/blob/main/codex-rs/app-server/README.md) is the supported integration boundary for clients; do not assume that a separate client wakes an already-open embedded Desktop task without an exact-task canary.

Private Desktop IPC, dedicated listener tasks, and quiet Git feelers are experimental maintainer constructions. They are never required for ordinary readiness or fallback, and their wire recipes and executable engines do not belong on the public path. Historical validation may record that an exact-task capability was observed, its build-bound caveats, and why the reusable listener was rejected. Further construction requires a fresh explicit Handler mailbox-infrastructure task after the durable arena-plus-runtime circuit breaker opens.

The scheduled firing's first action depends on the resolved transport:
...
A successful Codex schedule with a failed or unavailable required sync is `PARKED` before readiness or `DEGRADED` after a proven path fails. It must not reconcile a stale local copy and report “no message.” A detached watcher or stdout stream is not a post-turn wake bridge.
...
| Run a hidden Codex watcher after the turn ends | It may persist a result but cannot re-enter the task by file output or stdout alone | Native bounded Scheduled Collab, or honest Standard Exchange / `PARKED` fallback |
| Repair a failed listener during ordinary project work | Adapter work consumes the project and retries again after session restart | One durable arena-plus-runtime attempt, then circuit-break; only one exact Handler-authorized replacement run may clear it |
| Trust a silent listener or schedule because no failure arrived | A dead consumer cannot emit its own failure | Monitor uses its independent heartbeat; Scheduled Collab measures a genuine idle window and treats foreground overlap as suppressed, not missed |
