...
Validate a portable callsign and wait for deterministic name settlement before creating its workspace. Then write shared work product only under `workspace/<CALLSIGN>/artifacts/`; use `workspace/<CALLSIGN>/scratch/` for owner-mutable disposable files that never appear in delivery messages and still obey sensitivity/no-secrets rules. You may read a peer workspace, but never write or repair there. Artifact targets are create-once. Workspace presence is not delivery: send a causal inbox message with the workspace-relative path and full hash.

On pen transfer, map old path/hash to a new immutable continuation only when a predecessor artifact actually exists. With no predecessor artifact, send a causal `ack` naming the pen and the absence. Missing, ambiguous, or scratch-only predecessor evidence becomes `needs-audit`; never invent the old side or cite scratch. A displaced claimant's existing tree becomes `needs-audit: callsign-collision`; even the winner cannot inherit or write it until exact-path remediation closes the debt. Before `goodbye`, restore owned scratch to its empty baseline. A current Handler may authorize one finite, exact-path custodian action for collision residue, orphan/retirement cleanup, blocked-departure scratch, or a privacy incident; this is the only exception to ordinary create-once/owner-only mutation, records non-secret evidence, and grants no continuing ownership. An unsafe workspace write is refused without disabling a separately safe inbox. Treat a legacy flat `artifacts/` directory as read-only unless ownership is proved; never keep two active artifact homes.
