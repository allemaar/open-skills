...
> Apache-2.0. Portable shared doctrine bundled with this installed skill.
> The family members are `human-output`, `human-rewrite`, `human-draw`, and
> `human-merge`; each may be installed independently.
...
The source repository checks this roster against the shipped `human-*` folders.
That release check is repository evidence, not a prerequisite for using this
installed copy.
...
`node <skill-dir>/references/tools/human-output-check.mjs <draft-path>`.
The bundled checker grades the **mechanical half** of the contract.
...
## 4. Portable style floor

When `human-output` is installed, load it for the complete prose layer. When it
is unavailable, these five rules are the contained fallback:

1. Put the verdict first, including any caveat that would reverse it.
2. State the practical consequence, not only the underlying fact.
3. Label material claims as confirmed, judgement, or estimate; source confirmed claims.
4. State what was not checked.
5. Avoid shorthand the reader must decode and visuals that do no work.

Order by decision relevance. Fidelity is a floor and length is a target; never
shorten by dropping a number, scope boundary, uncertainty, or material gap.

## 5. Repository maintenance note
...
In the source repository, the shared footer and its coverage are checked by the
repository conformance workflow. Those checks maintain the pack; they are not
runtime dependencies of this installed skill.
